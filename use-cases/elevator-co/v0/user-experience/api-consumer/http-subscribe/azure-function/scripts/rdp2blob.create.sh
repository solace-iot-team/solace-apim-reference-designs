...
#
    ...
    templateFile="$scriptDir/rdp2blob.create.arm-template.json"
    parametersFile="$scriptDir/rdp2blob.create.parameters.json"
...
# Add Settings to Function App
  ...
  dataLakeStorageConnectionString=$( cat $outputDir/$outputFileCreateBlob | jq -r $jsonPath )
    if [[ $? != 0 ]]; then echo " >>> ERROR: reading $jsonPath from $outputDir/$outputFileCreateBlob"; exit 1; fi
    if [ "$dataLakeStorageConnectionString" == "null" ]; then echo ">>> ERROR: reading $jsonPath from $outputDir/$outputFileCreateBlob"; echo; exit 1; fi
    ...
    templateAppSettingsJSON=$(cat $templateAppSettingsFile | jq .)
      if [[ $? != 0 ]]; then echo " >>> ERROR: file not found: $templateAppSettingsFile."; exit 1; fi
    jsonPath=".STORAGE_CONTAINER_NAME"
      val=$(echo $templateAppSettingsJSON | jq -r $jsonPath)
      if [ "$val" == "null" ]; then echo ">>> ERROR: reading $jsonPath from $templateAppSettingsFile"; echo; exit 1; fi
...
echo " >>> Adding Function App Setttings ..."
  ...
  if [[ $? != 0 ]]; then echo " >>> ERROR: adding functionapp settings"; exit 1; fi
echo " >>> Success."
...
echo " >>> Deploying Function: $zipDeployFile ..."
  ...
  if [[ $? != 0 ]]; then echo " >>> ERROR: deploying function"; exit 1; fi
echo " >>> Success."
...
echo " >>> Retrieving Function Info ..."
  ...
  if [[ $? != 0 ]]; then echo ">>> ERROR: retrieving function app info."; exit 1; fi
echo " >>> Success."

echo " >>> Retrieving Function Keys ..."
  rdpAppInfoId=$(echo $rdpAppInfo | jq -r '.id')
  rdpAppKeysUri="$rdpAppInfoId/functions/$functionName/listKeys?api-version=2018-11-01"
  # echo "rdpAppInfoId='$rdpAppInfoId'"
  rdpAppkeys=$(az rest --method post --uri $rdpAppKeysUri)
  if [[ $? != 0 ]]; then echo ">>> ERROR: retrieving function keys."; exit 1; fi
  # echo $rdpAppkeys | jq .
